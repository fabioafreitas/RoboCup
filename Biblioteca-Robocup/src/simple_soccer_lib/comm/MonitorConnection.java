...
public class MonitorConnection {
	...
	public void connect() throws IOException {
		this.isActive = false;
		this.socket = new DatagramSocket(); //TODO: passar porta e address aqui? (e remover do send)
		...
		this.socket.receive(packet); //esta primeira mensagem é descartada
	...
	public boolean update(FieldPerception field, MatchPerception match) {
		...
		try {
			...
			if (System.currentTimeMillis() > this.lastMessage + 500) {
				System.err.println("MonitorCommm: too much time without message, stopping communicator");
				this.disconnect();
			}
			return false;
		
		} catch (IOException e) {
			System.err.println("MonitorCommm: error receiving data, stopping communicator");
			System.err.println("MonitorCommm: " + e.getMessage());
			this.disconnect();
			return false;
		}
