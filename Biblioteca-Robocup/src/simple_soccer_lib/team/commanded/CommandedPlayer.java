...
public class CommandedPlayer extends Thread {
	...
	private void doAction(String[] action){
		...
		if(action[0].equalsIgnoreCase("M")){
			try{
				...
				System.out.println("Comando inválido: "+comm+".\n"
			...
			System.out.println(action[0]+": "+matchPerc.getTime());
			...
			try{
				if(action[1].equalsIgnoreCase("A") || action[1].equalsIgnoreCase("B")){
					System.out.println(action[0]+":\n"+fieldPerc.getTeamPlayers(action[1].equalsIgnoreCase("A")? EFieldSide.LEFT : EFieldSide.RIGHT));
				}else{
					throw new Exception();
				}
			}catch(Exception e){
	...
	private void doAny(){
		if(matchPerc != null){
			...
			default:
				System.out.println("ESTADO INDEFINIDO: "+matchPerc.getState());
				break;
			}
		}
	}
	...
	private void runToBall(double erro) {
		if(matchPerc != null){
			while ((Math.abs(selfPerc.getPosition().getX() - fieldPerc.getBall().getPosition().getX()) > erro ||
				...
				boolean hasNew = updatePerceptions();
				/*if (hasNew) {
					double error = Math.abs(selfPerc.getPosition().getX() - fieldPerc.getBall().getPosition().getX()) +
							Math.abs(selfPerc.getPosition().getY() - fieldPerc.getBall().getPosition().getY()) ;
