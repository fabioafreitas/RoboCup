 ...
 * 2D simulada. Para isso, ele encapsula toda a parte de comunicação com o servidor.
 * 
 ...
 * uma nova percepção disponível quando o servidor enviar nova mensagem. Se for 
 * requisitada uma percepção no intervalo, será retornando null.
 * 
 */
public class PlayerCommander extends Thread {
	...
	//private Vector2D viewDirection; //direção absoluta da visão, necessária para algumas ações --> agora, acessa direto de "self"
	...
	public PlayerCommander(String teamName, String host, int port, boolean isGoalie) throws UnknownHostException {
		...
		this.communicator = new PlayerConnection(address, port, teamName); // InetAddress lança exceção
...
//  TODO: remover?
//	/**
//	 * Retorna lado do time --> pegar de self
//	 */
//	public EFieldSide getFieldSide() {
//		return this.fieldSide; //TODO: conferir: retorna sempre o lado inicial, mas não o atual? Bruno: Sim, sempre retorna o lado inicial. Detalhe: não há troca de lado no segundo tempo.
//	}
//	
//	/**
//	 * Retorna o número do jogador --> pegar de self
//	 */
//	public int getUniformNumber() {
//		return this.communicator.getInitialUniformNumber(); //TODO: conferir: retorna sempre o inicial? pode mudar? Bruno: em todos os testes feitos, nunca houve troca de número.
//	}
	...
	public void run() {
		...
		while (isActive()) {			
			...
			synchronized (this) {
				// 1. Recebe as perceções e faz o parsing delas
				// 2. Constroi uma representação de alto nível das percepções
				//    2.1 Calcular a posição e orientação absoluta do jogador (self) 
				//    2.2 Calcular as posições absolutas dos objetos móveis do campo (field)
				//	  2.3 Calcular as informações da partida (match)

				//TODO: problema: cria novas instâncias a cada iteração -- problema de memória
				//      ideia 1: manter uma instância fixa de cada um no monitor, e copiar para os atributos desta classe
				//      ideia 2: manter os atributos desta classe fixos (final?) e só retornar cópias
	...
	public PlayerPerception perceiveSelfBlocking() {
		...
		while (s == null) {   //TODO: atencao: risco de live lock, ideia: criar um semaforo apenas para as percepções
			try {
	...
	/**
	 * Faz o agente girar para uma direcao.
	 * Este metodo garante o envio do comando e, por isso, pode bloquear a execução por um tempo 
	 * de até SIMULATOR_CYCLE (100ms) para ser executado.
	 */
	...
	/**
	 * Move o jogador para a coordenada dada. Isso só pode ser feito em alguns
